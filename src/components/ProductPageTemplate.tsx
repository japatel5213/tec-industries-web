...
type MaterialComparison = { material: string; startWith: string; selectionGuidance: string; confirmBeforeSelection: string };
...
export default function ProductPageTemplate({ badge, name, tagline, description, image, specs, features, applications, relatedProducts, gallery, materialComparison }: ProductPageProps) {
  const productSchema = {
  ...
  return (
    <div className="bg-[#f3f5f2]">
      ...
      {materialComparison?.length ? <section className="border-y border-[#25404a] bg-[#0f171f] py-[clamp(4rem,7vw,7rem)] text-white"><div className="container-xl"><div className="max-w-3xl"><p className="section-eyebrow text-[#55ca92]">Material selection guide</p><h2 className="mt-4 font-[var(--font-display)] text-[clamp(3rem,5vw,5.6rem)] font-extrabold uppercase leading-[0.82] tracking-[-0.045em]">Select the service.<br />Then the material.</h2><p className="mt-6 max-w-2xl text-[0.98rem] leading-7 text-white/68">This is a project-conversation guide, not a substitute for the selected manufacturer’s pressure-temperature, chemical-compatibility, dimensions or system-approval documents.</p></div><div className="mt-10 overflow-x-auto border border-white/15"><table className="min-w-[920px] w-full border-collapse text-left"><caption className="sr-only">Comparison of PPR, PPH, PP-RCT and PPCH material-selection guidance</caption><thead className="bg-[#173c36]"><tr><th className="px-5 py-4 font-[var(--font-mono)] text-[0.63rem] uppercase tracking-[0.12em] text-[#55ca92]">Material</th><th className="px-5 py-4 font-[var(--font-mono)] text-[0.63rem] uppercase tracking-[0.12em] text-[#55ca92]">Start the discussion with</th><th className="px-5 py-4 font-[var(--font-mono)] text-[0.63rem] uppercase tracking-[0.12em] text-[#55ca92]">TEC selection guidance</th><th className="px-5 py-4 font-[var(--font-mono)] text-[0.63rem] uppercase tracking-[0.12em] text-[#55ca92]">Confirm before selection</th></tr></thead><tbody>{materialComparison.map((item, index) => <tr key={item.material} className={index % 2 === 0 ? 'bg-white/[0.035]' : 'bg-white/[0.075]'}><th scope="row" className="border-t border-white/10 px-5 py-5 font-[var(--font-display)] text-[1.55rem] font-bold uppercase leading-none text-white">{item.material}</th><td className="border-t border-white/10 px-5 py-5 text-sm leading-6 text-white/72">{item.startWith}</td><td className="border-t border-white/10 px-5 py-5 text-sm leading-6 text-white/72">{item.selectionGuidance}</td><td className="border-t border-white/10 px-5 py-5 text-sm leading-6 text-white/72">{item.confirmBeforeSelection}</td></tr>)}</tbody></table></div></div></section> : null}
